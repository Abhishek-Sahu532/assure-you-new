...
//google auth
export const google = (userData) => async (dispatch) => {
  try {
    dispatch({ type: REGISTER_USER_REQUEST });
    const config = { headers: { "Content-Type": "application/json" } }; //multipart/form-data instead of json type, because in registration images also included

    const { data } = await axios.post(`/api/v1/google`, userData, config);
    dispatch({ type: REGISTER_USER_SUCCESS, payload: data.user });
